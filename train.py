...
from waternet.net import WaterNet, CLIPContrastive, CLIPFeature
...
TRAIN_METRICS_NAMES = ["mse", "ssim", "psnr", "perceptual_loss", "loss", "CLIP_loss", "CLIP_mse"]
VAL_METRICS_NAMES = ["mse", "ssim", "psnr", "perceptual_loss", "CLIP_loss", "CLIP_mse"]
...
def eval_one_epoch(model, val_dataloader, device):
    ...
    with torch.no_grad():
        for _, next_data in pbar:
            ...
            clip_contrastive_loss = clip_contrastive(out)
            clip_feature_loss = clip_feature(out, rgb_ten)
            ...
            epoch_metrics["CLIP_loss"] += clip_contrastive_loss.item()
            epoch_metrics["CLIP_mse"] += clip_feature_loss.item()
...
def train_one_epoch(
    ...
    for idx, next_data in pbar:
        ...
        clip_contrastive_loss = clip_contrastive(out)
        clip_feature_loss = clip_feature(out, rgb_ten, [1., 1., .8, .6, .4])
        mse = torch.mean(torch.square(255 * (out - ref_ten)))

        loss = 0.5 * (clip_feature_loss * 0.05 + clip_contrastive_loss) \
        + 0.5 * (mse + perceptual_loss * 0.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=10, help="(Optional) Num epochs, defaults to 400"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    raw_df = pd.read_csv(projectroot / "datasets/raw_ground_truth.csv")
    ref_df = pd.read_csv(projectroot / "datasets/reference_ground_truth.csv")
    dataset = UIEBDataset(
        raw_df,
        ref_df,
        im_height=im_height,
    ...
    clip_contrastive = CLIPContrastive()
    clip_contrastive.to(device)
    clip_contrastive.eval()

    clip_feature = CLIPFeature()
    clip_feature.to(device)
    clip_feature.eval()
    ...
    for i in range(num_epochs):
        ...
        if eval_metrics["CLIP_loss"] + 0.3 * train_metrics["CLIP_loss"] < clip_loss:
            clip_loss = eval_metrics["CLIP_loss"] + 0.3 * train_metrics["CLIP_loss"]
            torch.save(model.state_dict(), savedir / "best.pt")
